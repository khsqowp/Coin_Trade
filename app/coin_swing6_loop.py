"""코인 6종목 롱숏 스윙 페이퍼봇 상시 실행 루프.

TRX 봇(trx_swing_loop.py)과 같은 구조 — 사이클 단위로 예외를 흡수해 루프 전체가 죽지 않게
한다. 이 봇은 완전 페이퍼(가상자본)라 사이클 주기를 더 타이트하게 잡아도 실계좌 API 부하
걱정이 없다(공개 시세 조회만 함, 키 불필요).
"""
...
CYCLE_TIMEOUT_SECONDS = int(os.environ.get("COIN_SWING6_CYCLE_TIMEOUT_SECONDS", "90"))


def main() -> None:
    print(f"코인 6종목 롱숏 스윙 페이퍼봇 루프 시작 (사이클 주기 {CHECK_INTERVAL_SECONDS}초)", flush=True)
    while True:
        try:
            run_with_timeout(
